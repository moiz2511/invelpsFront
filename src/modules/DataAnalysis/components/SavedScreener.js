...
const DAScreener = () => {
    ...
    async function getCompaniesOnEffect(exchanges,sectors,industry) {
        setShowCompaniesDataProgress(true);
        setCircularProgress(true);
        const body = { country: countryFilter.map((data => data.country)), exchange: exchanges.map((data => data.exchange)), sector: sectors.map((data => data.sector)), industry: industry.map((data) => data.industry) }
        await restClient.getCompanies(body)
            .then((response) => {
                console.log(response)
                getLivePricesAndMarketCap(response.data.resp_data)
    ...
    useEffect(()=>{
        ...
        const getScreenerById = async(_id) => {
            ...
            .then((response) => {
                ...
                if(response.data.screener.sectorFilter!=""){
                    setSectorFilter(response.data.screener.sectorFilter)
                }
                if(response.data.screener.industryFilter!=""){
                    setIndustryFilter(response.data.screener.industryFilter)
                }
                setPeriodFilter(response.data.screener.periodFilter)
                getCompaniesOnEffect(response.data.screener.exchangeFilter, response.data.screener.sectorFilter ==""?[]:response.data.screener.sectorFilter, response.data.screener.industryFilter ==""?[]:response.data.screener.industryFilter)
