...
const InvestingStyle = () => {
  ...
  async function getInvestingStyles() {
    ...
    await restClientinvestingStyle
      .getInvestingStyles(body)
      .then((response) => {
        console.log(response);
  ...
  const handlePageRedirect = (row) => {
    ...
    let model = row.strategy_name
      ? "&model=" + encodeURIComponent(row.strategy_name)
      : "";
    window.open(
      Constants.APP_BASE_URL +
        "/context/screenmodel?style=" +
        style +
        mentor +
        model,
      "_blank",
                    ...
                    .map((row, index) => {
                      return (
                        ...
                        >
                          ...
                          <StyledTableCell>{row.philosophy}</StyledTableCell>
                          <StyledTableCell>
                            <Link onClick={() => handlePageRedirect(row)}>
                              {row.strategy_name}
                            </Link>
                          </StyledTableCell>
                          <StyledTableCell>{row.periodRange}</StyledTableCell>
                          <StyledTableCell>{row.fundReturn}</StyledTableCell>
                          <StyledTableCell>{row.marketReturn}</StyledTableCell>
                          <StyledTableCell>{row.riskTolerance}</StyledTableCell>
