...
const ContextFundamentalChart = () => {
  ...
  useEffect(() => {
    ...
    if (tool !== null && tool !== "") {
      getMeasuresDropDown(tool);
    }
    if (measure !== null && measure !== "") {
      getCategoriesDropDown(tool, measure);
    }
    if (category !== null && category !== "") {
      getMetricsDropDown(tool, measure, category);
    }
  ...
  const setToolFilterHandler = (event) => {
    ...
    getMeasuresDropDown(event.target.value);
  }

  const setMeasureFilterHandler = (event) => {
    setMeasureFilter(event.target.value);
    getCategoriesDropDown(toolFilter, event.target.value);
  }

  const setCategoryFilterHandler = (event) => {
    setCategoryFilter(event.target.value);
    getMetricsDropDown(toolFilter, measureFilter, event.target.value);
  }
  ...
  async function getMeasuresDropDown(tool) {
    await restClient.getMeasuresByTool({ tool })
      .then((response) => {
  ...
  async function getCategoriesDropDown(tool, measure) {
    await restClient.getCategoriesByMeasure({ tool, measure })
      .then((response) => {
        console.log(response)
  ...
  async function getMetricsDropDown(tool, measure, category) {
    await restClient.getMetricsByCategory({ tool, measure, category })
      .then((response) => {
        console.log(response)
          ...
          <Grid container
            ...
            <Grid item sx={{ marginTop: 0.75 }}>
              ...
              >
                ...
                {/* <MenuItem value="ratios">
                  ratios
                </MenuItem>
                <MenuItem value="financials">
                  financials
                </MenuItem>
                <MenuItem value="vertical analysis">
                  vertical analysis
                </MenuItem>
                <MenuItem value="horizontal analysis">
                  horizontal analysis
                </MenuItem>
                <MenuItem value="customratios">
                  CustomRatios
                </MenuItem> */}
            ...
            <Grid item sx={{ marginTop: 0.75 }}>
              ...
              >
                ...
                {measureDropDownValues.map((item) => (
                  <MenuItem key={item.measure} value={item.measure}>
                    {item.measure}
                  </MenuItem>
                ))}
              </TextField>
            ...
            <Grid item sx={{ marginTop: 0.75 }}>
              ...
              >
                <MenuItem key="default-value" disabled value="">
                  <em>Select Category</em>
                </MenuItem>
                {categoryDropDownValues.map((item) => (
                        ...
                        >
                          ...
                          </StyledTableCell>
                          {
                            chartApiResponse.date_range.map((field, index) => {
                              const unitSymbol = chartInputData.yUnit === "percent" ? "%" : ""
                              return (
                                <StyledTableCell key={index}>
                                  {row[field]}{unitSymbol}
                                </StyledTableCell>
                              );
                            })
                          }
...
export default ContextFundamentalChart;