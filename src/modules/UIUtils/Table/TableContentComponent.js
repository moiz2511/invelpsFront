...
function CustomizedTable(props) {
    ...
    const navigateToPageOnClick = (data) => {
        if (data.page === 'profile') {
            window.open(Constants.APP_BASE_URL + "/dataanalysis/profile?company=" +
                encodeURIComponent(data.company_name) + "&exchange=" + encodeURIComponent(data.exchange), '_blank', 'noopener,noreferrer')
        } else if (data.page === 'financials') {
            window.open(Constants.APP_BASE_URL + "/dataanalysis/financials?company=" +
                encodeURIComponent(data.company_name) + "&exchange=" + encodeURIComponent(data.exchange), '_blank', 'noopener,noreferrer')
        } else if (data.page === 'keymetricsttm') {
            window.open(Constants.APP_BASE_URL + "/dataanalysis/keymetrics?company=" +
                encodeURIComponent(data.company_name) + "&exchange=" + encodeURIComponent(data.exchange), '_blank', 'noopener,noreferrer')
        } else if (data.page === 'marketData') {
            window.open(Constants.APP_BASE_URL + "/dataanalysis/marketdata?company=" +
                encodeURIComponent(data.company_name) + "&exchange=" + encodeURIComponent(data.exchange), '_blank', 'noopener,noreferrer')
        } else if (data.page === 'deepanalysis') {
            let style = props.data.style ? "&style=" + encodeURIComponent(props.data.style) : "";
            let mentor = props.data.mentor ? "&mentor=" + encodeURIComponent(props.data.mentor) : "";
            let screenModel = props.data.screenModel ? "&screenModel=" + encodeURIComponent(props.data.screenModel) : "";
            window.open(Constants.APP_BASE_URL + "/context/analysismodel?company=" +
                encodeURIComponent(data.company_name) + "&exchange=" + encodeURIComponent(data.exchange) + style + mentor + screenModel, '_blank', 'noopener,noreferrer')
            ...
            let from = data.model_from ? "&from=" + encodeURIComponent(getTextToYeaaNotation(data.model_from)) : "";
            let to = data.model_to ? "&to=" + encodeURIComponent(getTextToYeaaNotation(data.model_to)) : "";
            let range = data.range ? "&range=" + encodeURIComponent(data.range) : "";
            window.open(Constants.APP_BASE_URL + "/dataanalysis/fundamentalchart?company=" +
                encodeURIComponent(props.company.value) + "&exchange=" + encodeURIComponent(data.exchange) + tool + measure + category + from + to + range, '_blank', 'noopener,noreferrer')
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - props.tableRows.length) : 0;
    ...
    let tableData = props.enableSorting ? stableSort(props.tableRows, getComparator(order, orderBy)) : props.tableRows;
    ...
    return (
        <Box sx={{ width: '100%' }}>
                    ...
                    <TableHead>
                        <TableRow>
                            ...
                            {props.headCells.data.map((headCell) => (
                                ...
                                >
                                    {props.enableSorting && !headCell.isDropDown && <TableSortLabel
                                    ...
                                    {(!props.enableSorting || headCell.isDropDown) && headCell.label}
                                </StyledTableCell>
                            ))}
                        </TableRow>
                    </TableHead>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {props.headCells.data.map((item) => {
                                            row.page = item.id
                                            return (
                                                <StyledTableCell key={item.id + row.id} padding='normal' sx={{
                                                    '& .MuiTextField-root': { minWidth: '20ch' },
                                                }} >
                                                    {/* {console.log(item)} */}
                                                    {!item.isValueLink && row[item.id]} {props.showTool==1 ? <BigBoxTooltip item={item} row={row} keyMatric={true} /> : props.showTool==0 ? <BigBoxTooltip item={item} keyMatric={false} row={row.details[0]} />:''} 
            ...
            {!props?.disablePagination && <TablePagination
